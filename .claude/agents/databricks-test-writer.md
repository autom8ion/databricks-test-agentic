...
1. Decide which layer the request belongs to:
   ...
   - A wall-clock duration/SLA check on a pipeline operation at realistic volume → **Layer 6**, extend `tests/integration/test_performance.py` — reuse its `_timed()` context manager and `src/dbx_tests/bulk_data.py` for volume, add a new SLA constant, and keep it behind the `DBX_TESTS_RUN_PERF` env-var skip.
