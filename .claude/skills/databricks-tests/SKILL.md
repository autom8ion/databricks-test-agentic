...
1. `python3 -m venv .venv-unit && source .venv-unit/bin/activate` (skip venv creation if it already exists — just activate it).
2. `pip install -r requirements-unit.txt`.
...
2. `python3 -m venv .venv && source .venv/bin/activate` (skip if it exists), `pip install -r requirements.txt`.
