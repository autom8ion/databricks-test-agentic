...
- **A test that needs real volume to be meaningful (data-skew, performance, large-scale reconciliation) belongs behind an env-var skip** (see `DBX_TESTS_RUN_BULK` in `test_reconciliation_bulk.py`, `DBX_TESTS_RUN_PERF` in `test_performance.py`), not in the default integration run — it's meant for a nightly job, not every merge.
- **Performance/SLA checks (`test_performance.py`) time an operation with the `_timed()` context manager and assert against a module-level SLA constant** — same pattern as `FRESHNESS_SLA` in `test_freshness.py`. A single failure there is often cluster cold-start, not a regression: re-run before concluding it's real, and never tighten/loosen an SLA constant to make a single run's result look right.
